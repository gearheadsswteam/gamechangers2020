...
public class RedRingCase1AutonomousOpModeRR {
    ...
    public void executeOpMode() {
        Trajectory traj1 = mecanumDriveRR.trajectoryBuilder(initPos, 0).splineTo(new Vector2d(35, -33), -0.4).build();
        Trajectory traj2 = mecanumDriveRR.trajectoryBuilder(new Pose2d(35, -33, -0.4), Math.PI - 0.4).splineToLinearHeading(new Pose2d(-2, -36, 0.35), 0.35).build();
        Trajectory traj3 = mecanumDriveRR.trajectoryBuilder(new Pose2d(-2, -36, 0.55), 0.55).splineTo(new Vector2d(-56, -11), -Math.PI / 3).splineTo(new Vector2d(-49, -23), -Math.PI / 3).splineTo(new Vector2d(8, -50), 0).build();
